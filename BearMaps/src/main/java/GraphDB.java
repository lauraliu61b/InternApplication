...
//    }
     ...
     */
    private void clean() {
        Map<Long, Node> newNodes = new HashMap<>();
        for (Map.Entry<Long, Node> entry : nodes.entrySet()) {
            if (!entry.getValue().adjacencies.isEmpty()) {
    ...
    long closest(double lon, double lat) {
        ...
        for (Map.Entry<Long, Node> entry : nodes.entrySet()) {
            Double currentDistance = this.distance(entry.getValue().lon,
                    entry.getValue().lat, lon, lat);
    ...
    public void addEdge(Node s1, Node s2) {
        s1.addAdj(s2);
    }
