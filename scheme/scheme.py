...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        while temp != nil:
            vals_size += 1
            temp = temp.second
        if vals_size != formals_size:
            raise SchemeError('SchemeError!!')
        else:
            for i in self.bindings:
                child.define(i, self.bindings[i])
            while formals != nil:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        if isinstance(expressions.second.first, Pair):
        ...
        t = ''
        t += target.first
        symbols = copy_pair(expressions.first.second)
        body = copy_pair(expressions.second)
        env.define(t, do_lambda_form(Pair(symbols, body), env))
        return t
        # END PROBLEM 10
    else:
...
def copy_pair(pair):
    if pair == nil:
        ...
        return Pair(pair.first, copy_pair(pair.second))
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        ...
        while expressions != nil:
            ...
            if result == False and type(result) == bool:
                return False
            expressions = expressions.second
    return result
...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        ...
        while expressions != nil:
            result = scheme_eval(expressions.first, env)
            if result or (type(result) != bool and result == False):
                return result
            expressions = expressions.second
    return result
    # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    bindings.map(lambda b: check_form(b, 2, 2))
    formals = bindings.map(lambda p: p.first)
    args = bindings.map(lambda p: scheme_eval(p.second.first, env))
    check_formals(formals)        
